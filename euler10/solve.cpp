...
uint64_t sumOfPrimesBelow(const uint64_t max_prime, const bool verbose) {
  if (verbose) {
    cout << endl;
  }
  ...
  while (next_prime < max_prime) {
    ...
    if (verbose && count_primes % 20000 == 0) {
      cout << "\033[A\033[K" << (int)(100 * next_prime / max_prime) << "% ("
           << count_primes << ")" << endl;
    }
    count_primes++;
  }
...
uint64_t sumOfNbPrimes(const uint64_t nb_primes, const bool verbose) {
  if (verbose) {
    cout << endl;
  }
  uint64_t sum_primes = 0;
  ...
  for (uint64_t count_primes = 0; count_primes < nb_primes; count_primes++) {
    ...
    sum_primes += prime;
    sum_primes %= 10'000'000u;
    if (verbose && count_primes % 100000 == 0) {
      cout << "\033[A\033[K" << (int)(100 * count_primes / nb_primes) << "% ("
           << count_primes << ")" << endl;
    }
  }
  return sum_primes;
...
int main(int argc, char *argv[]) {
  ...
  cout << sumOfPrimesBelow(2'000'000, true) << endl << endl;
  cout << "Computing the sum mod 7 of the  8,971,406 first primes (euler)"
       << endl;
  cout << sumOfNbPrimes(8'971'406, true) << endl;
